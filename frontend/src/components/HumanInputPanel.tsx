...
export default function HumanInputPanel({ weekId, pipelineState, onSubmitted }: { weekId: string, pipelineState: any, onSubmitted: () => void }) {
  ...
  useEffect(() => {
     if (isResearchInterrupt) {
         ...
         const preferredName = isDeepResearchInterrupt && pendingTopicId
           ? `deep_research_prompt_${pendingTopicId}.md`
           : "research_prompts.md";
         const fallbackName = "deep_research_prompts.md";
         ...
         getArtifact(weekId, phase, preferredName)
           ...
           .catch(async () => {
             if (isDeepResearchInterrupt) {
               try {
                 const fallback = await getArtifact(weekId, phase, fallbackName);
                 setPromptContent(fallback.content);
                 return;
               } catch {
                 // Fall through to unified error below.
               }
             }
             setPromptContent("Failed to load prompt generated by the AI.");
           });
     }
  }, [actionType, weekId, isResearchInterrupt, isDeepResearchInterrupt, pendingTopicId]);
